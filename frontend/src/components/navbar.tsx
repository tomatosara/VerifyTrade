  ...
  return (
    <nav className="flex items-center justify-between px-6 py-4 bg-white shadow-sm relative">
      ...
      <div className="flex items-center space-x-2">
        ...
        <Link
          to="/"
          className="font-semibold text-lg text-gray-800 hover:text-[var(--color-primary)] transition-colors"
        >
          身分驗證交易平台
        </Link>
      </div>
      ...
      <div className="hidden md:flex space-x-3 items-center">
        <Link to="/myaccount" className="text-gray-700 text-sm hover:text-[var(--color-primary)] transition">
          我的帳號
        </Link>
        <Link
          to="/login"
          className="bg-[var(--color-primary)] text-sm text-white px-4 py-2 rounded-full hover:bg-[var(--color-primary)]/80 transition"
        >
          註冊/登入
        </Link>
      </div>
      ...
      {/* 🔹 手機版展開選單 */}
      {isOpen && (
        <div className="absolute top-full left-0 w-full bg-white shadow-md flex flex-col items-center py-4 space-y-3 md:hidden z-50">
          ...
          <Link
            to="/login"
            className="bg-[var(--color-primary)] text-white px-5 py-2 rounded-full hover:bg-[var(--color-primary)]/80"
          >
            登入 / 註冊
          </Link>
        </div>
