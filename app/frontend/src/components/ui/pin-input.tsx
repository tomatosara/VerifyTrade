...
const Group = ({ inputClassName, containerClassName, width, maxLength = 8, ...props }: GroupProps) => {
...
const Slot = ({ index, className, ...props }: ComponentPropsWithRef<"div"> & { index: number }) => {
    ...
    return (
        <div
            ...
            className={cx(
                "relative flex items-center justify-center rounded-xl bg-primary text-center text-placeholder_subtle shadow-xs ring-1 ring-gray-300 transition-[box-shadow,background-color] duration-100 ease-linear ring-inset",
                sizes[size],
                isFocused && slot?.isActive && "ring-2 ring-[var(--color-primary)] outline-2 outline-[var(--color-primary)] outline-offset-0",
                slot?.char && "text-brand-tertiary_alt ring-2 ring-[var(--color-primary)]",
